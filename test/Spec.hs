...
readBS = S.pack . go
  where go s = case readHex (dropWhile isSpace s) of
          []          -> []
          ((x, s'):_) -> x : go s'
        isSpace x = x `elem` (" \t\r\n" :: String)

keyVect = replicate 12 (S.pack [0xc0..0xcf])
       ++ replicate 12 (S.pack [ 0xd7, 0x82, 0x8d, 0x13
                               , 0xb2, 0xb0, 0xbd, 0xc3
                               , 0x25, 0xa7, 0x62, 0x36
                               , 0xdf, 0x93, 0xcc, 0x6b])

nonceVect = map readBS [
...
readInput k = S.splitAt k . readBS

-- input vect are `adata + message`
inputVect = map (uncurry readInput) [
...
ccmTestData = zipWith4 combine keyVect nonceVect inputVect outputVect
  where
    combine k n (adata, msg) (m, o) = CcmTestData m 2 k msg n adata o

doCcmTest (x, (CcmTestData m l k i n a o)) = it ("testcase: " ++ show x) ((a <> ccmEncrypt m l key i n a) `shouldBe` o)
